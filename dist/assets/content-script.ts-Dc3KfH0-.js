...
`).replace(/```[\s\S]*?```/g,e=>e.replace(/```/g,``).trim()).replace(/\*\*(.+?)\*\*/g,`$1`).replace(/__(.+?)__/g,`$1`).replace(/`([^`]+?)`/g,`$1`).replace(/(?<!\*)\*([^*\n]+?)\*(?!\*)/g,`$1`).replace(/(?<!_)_([^_\n]+?)_(?!_)/g,`$1`).replace(/^[ \t]*[-*•][ \t]+/gm,`• `).trim()}async function g(e){try{return await navigator.clipboard.writeText(e),!0}catch{try{let t=document.createElement(`textarea`);t.value=e,t.style.position=`fixed`,t.style.top=`-1000px`,t.style.opacity=`0`,document.body.appendChild(t),t.focus(),t.select();let n=document.execCommand(`copy`);return t.remove(),n}catch{return!1}}}chrome.runtime.onMessage.addListener(e=>{if(e.type===`TOQAN_CONTEXT_RESPONSE`){let t=e;d({type:t.type,query:t.query,response:t.response})}else if(e.type===`TOQAN_CONTEXT_ERROR`){let t=e;d({type:t.type,error:t.error})}else e.type===`OMBRE_ADD_TO_CHAT`&&`text`in e&&e.text&&c?.(e.text)});var _=`ombre-ai-edge-panel-host`,v=`toqan_edge_conversations`;function y(){return`edge-${Date.now()}-${Math.random().toString(36).slice(2,8)}`}function b(e){let t=e.trim().replace(/\s+/g,` `);return t.length>42?`${t.slice(0,42)}…`:t||`New chat`}function x(e){let t=Date.now()-e,n=Math.floor(t/6e4);if(n<1)return`Just now`;if(n<60)return`${n}m ago`;let r=Math.floor(n/60);if(r<24)return`${r}h ago`;let i=Math.floor(r/24);return i<7?`${i}d ago`:new Date(e).toLocaleDateString(void 0,{month:`short`,day:`numeric`})}function S(){if(window.self!==window.top||document.getElementById(_))return;let t=document.createElement(`div`);t.id=_,document.documentElement.appendChild(t);let o=t.attachShadow({mode:`open`}),s=document.createElement(`style`);s.textContent=`
    :host { all: initial; }
  ...
  `,t.append(r,i,s);let l=s.querySelector(`.card-body`),u=s.querySelector(`.card-footer`),d=s.querySelector(`.card-close`),m=``,_=null,v=!1,y=null,b=0,x=0,S=null;function T(){i.classList.remove(`visible`)}function E(){s.classList.remove(`visible`),S=null}function D(e){let t=e instanceof Element?e:e?.parentElement??null;for(;t;){if(t.id===`ombre-ai-edge-panel-host`||t.id===`ombre-ai-context-panel-host`||t.id===C)return!0;t=t.parentElement}return!1}function O(e){let t=e instanceof Element?e:e?.parentElement??null;for(;t;){if(t instanceof HTMLElement&&(t.isContentEditable||t.tagName===`TEXTAREA`||t.tagName===`INPUT`))return!0;t=t.parentElement}return!1}let k=new Set([`text`,`search`,`url`,`tel`,`email`,`password`,``]);function A(){let e=document.activeElement;if(D(e))return null;let t=e instanceof HTMLTextAreaElement,n=e instanceof HTMLInputElement&&k.has(e.type);if(!t&&!n)return null;let r=e,i=r.selectionStart,a=r.selectionEnd;return i==null||a==null||a<=i?null:{el:r,text:r.value.slice(i,a),start:i,end:a}}function j(e,t,n,r){let i=t.top-r-8,a=t.left+t.width/2-n/2;i<8&&(i=t.bottom+8),a<8&&(a=8),a+n>window.innerWidth-8&&(a=window.innerWidth-n-8),i+r>window.innerHeight-8&&(i=Math.max(8,window.innerHeight-r-8)),e.style.top=`${i}px`,e.style.left=`${a}px`}function M(e){if(e.width===0&&e.height===0){T();return}i.classList.add(`visible`),requestAnimationFrame(()=>{j(i,e,i.offsetWidth,i.offsetHeight)})}function N(){if(o||s.classList.contains(`visible`))return;let e=A();if(e){m=e.text.trim(),_=null,v=!0,y=e.el,b=e.start,x=e.end,M(e.el.getBoundingClientRect());return}let t=window.getSelection(),n=t?.toString().trim()??``;if(!n||!t||t.rangeCount===0){T();return}let r=t.getRangeAt(0);if(D(r.commonAncestorContainer)){T();return}m=n,_=r.cloneRange(),v=O(r.commonAncestorContainer),y=null,M(r.getBoundingClientRect())}let P;function F(){window.clearTimeout(P),P=window.setTimeout(N,120)}document.addEventListener(`selectionchange`,F),document.addEventListener(`mouseup`,F),document.addEventListener(`keyup`,e=>{(e.shiftKey||e.key===`Shift`)&&F()}),document.addEventListener(`mousedown`,e=>{D(e.target)||(T(),E())}),window.addEventListener(`scroll`,T,!0),document.addEventListener(`keydown`,e=>{e.key===`Escape`&&(T(),E())}),d.addEventListener(`click`,E);function I(){l.innerHTML=`<div class="card-loading"><span></span><span></span><span></span></div>`,u.style.display=`none`}function L(e,t){if(l.innerHTML=t?`<div class="error-text">${f(e)}</div>`:p(e),t){u.style.display=`none`;return}u.style.display=`flex`,u.innerHTML=`
    ...
    `,u.querySelector(`[data-act="copy"]`)?.addEventListener(`click`,async t=>{let n=t.currentTarget,r=n.innerHTML;n.innerHTML=await g(h(e))?`<svg viewBox="0 0 24 24"><path d="M20 6 9 17l-5-5"/></svg> Copied`:`<svg viewBox="0 0 24 24"><path d="M18 6 6 18M6 6l12 12"/></svg> Couldn't copy`,n.disabled=!0,setTimeout(()=>{n.innerHTML=r,n.disabled=!1},1600)}),u.querySelector(`[data-act="replace"]`)?.addEventListener(`click`,()=>{let t=h(e);if(y)R(y,b,x,t);else if(_)try{let e=window.getSelection();e?.removeAllRanges(),e?.addRange(_),document.execCommand(`insertText`,!1,t)}catch{g(t)}E()})}function R(e,t,n,r){let i=e instanceof HTMLTextAreaElement?window.HTMLTextAreaElement.prototype:window.HTMLInputElement.prototype,a=Object.getOwnPropertyDescriptor(i,`value`)?.set,o=e.value.slice(0,t)+r+e.value.slice(n);a?a.call(e,o):e.value=o,e.dispatchEvent(new Event(`input`,{bubbles:!0}));let s=t+r.length;e.focus();try{e.setSelectionRange(s,s)}catch{}}function z(e){I();let t=`sel-${Date.now()}-${Math.random().toString(36).slice(2,8)}`;S=t,n({type:`TOQAN_CHAT`,messages:[{id:`1`,role:`user`,content:e,createdAt:Date.now()}],conversationId:t}).catch(e=>{S===t&&L(e.message||`Something went wrong.`,!0)})}function B(){u.style.display=`none`;let e=m.length>140?`${m.slice(0,140)}…`:m;l.innerHTML=`
    ...
    `;let t=l.querySelector(`.addmore-input`),n=l.querySelector(`.addmore-submit`);t.focus();let r=()=>{let e=t.value.trim();z(e?`Here is a piece of text:\n\n"""${m}"""\n\nRegarding this text, the reader wants to know more about the following, so answer it clearly using the text as context: ${e}`:w.addmore(m))};n.addEventListener(`click`,r),t.addEventListener(`keydown`,e=>{e.key===`Enter`&&!e.shiftKey&&(e.preventDefault(),r())})}function V(e){if(!m||o)return;let t=y?y.getBoundingClientRect():_?.getBoundingClientRect();if(T(),s.classList.add(`visible`),requestAnimationFrame(()=>{t&&j(s,t,320,e===`addmore`?210:200)}),e===`addmore`){B();return}z(w[e](m))}a.push(()=>{T(),E()}),chrome.runtime.onMessage.addListener(e=>{!e.conversationId||e.conversationId!==S||(e.type===`TOQAN_REPLY`?L(e.reply??``,!1):e.type===`TOQAN_ERROR`&&L(e.error??`Unknown error`,!0))}),i.querySelectorAll(`.tbtn[data-action]`).forEach(e=>{e.addEventListener(`click`,()=>V(e.dataset.action))}),i.querySelector(`.addchat`).addEventListener(`click`,()=>{o||!m||(T(),window.self===window.top&&c?c(m):n({type:`OMBRE_ADD_TO_CHAT`,text:m}).catch(()=>{}))})}document.readyState===`loading`?document.addEventListener(`DOMContentLoaded`,()=>{S(),T()}):(S(),T());